...
def extract_frames(video_path , frames_per_second=1, quality=95):
    ...
    output = f"SYSTEM/Data/{video_name}"
    return output
def extract_frames(video_path , frames_per_second=1, quality=95):
    if not os.path.exists(video_path):
        print(f"{video_path} not found")
...
def video_text(name, data_dir="SYSTEM/Data", frames_per_second=1, quality=95 ):
    ...
    for item in videofolder :
        name1 = os.path.join(videofolder,item)
        image_text(item , folder=videofolder)
if __name__ == "__main__":
    extract_frames("SYSTEM/Data/xx.mp4")
